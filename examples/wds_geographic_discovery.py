...
class WDSGeographicDiscovery:
    ...
    async def test_member_id(self, member_id: int, verbose: bool = False) -> dict[str, Any] | None:
        """
        ...
        try:
            data = await self.client.get_data_from_cube_pid_coord_and_latest_n_periods(
                product_id=self.population_cube_id,
    ...
    async def discover_range(self, start_id: int, end_id: int, verbose: bool = True) -> list[dict[str, Any]]:
        """
        ...
        for member_id in range(start_id, end_id + 1):
            ...
            if verbose and total_tested % 100 == 0:
                print(f"   Tested {total_tested}/{end_id - start_id + 1} IDs, found {len(valid_locations)} valid")
            
            location_info = await self.test_member_id(member_id, verbose=False)
            if location_info:
                valid_locations.append(location_info)
                if verbose:
                    print(f"✅ Found {member_id}: Population = {location_info['population_2021']:,}")
    ...
    async def discover_major_cities(self) -> list[dict[str, Any]]:
        ...
        for city in major_cities:
            print(f"   Member ID {city['member_id']}: {city['population_2021']:,} people")
    ...
    def generate_enum_code(self, locations: list[dict[str, Any]]) -> str:
        ...
        sorted_locations = sorted(locations, key=lambda x: x['population_2021'], reverse=True)
        ...
        for loc in sorted_locations:
            ...
            enum_entries.append(f"    {enum_name} = {member_id}  # Population: {population:,}")
...
async def main():
    ...
    parser.add_argument('--major-cities', '-m', action='store_true', 
                       help='Discover major cities (population > 100K)')
    parser.add_argument('--test-id', '-t', type=int,
                       help='Test a specific member ID')
    parser.add_argument('--output', '-o', type=str, default='scratch/discovered_locations.json',
                       help='Output file for results (default: scratch/discovered_locations.json)')
    parser.add_argument('--generate-enum', '-g', action='store_true',
    ...
    if all_results:
        ...
        print(f"   • Largest location: Member ID {max_pop['member_id']} ({max_pop['population_2021']:,} people)")
