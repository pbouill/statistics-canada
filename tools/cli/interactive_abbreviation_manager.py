...
try:
    ...
    MORPHOLOGICAL_TESTING_AVAILABLE = True
except ImportError:
    MORPHOLOGICAL_TESTING_AVAILABLE = False
    print("⚠️  SubstitutionEngine not available - morphological analysis will be limited")
...
class InteractiveAbbreviationManager:
    ...
    def __init__(self):
        ...
        self.changes_made = []
        self.subs_engine = SubstitutionEngine() if MORPHOLOGICAL_TESTING_AVAILABLE else None
    ...
    def load_word_tracking_data(self, tracking_file: Path) -> Optional[WordTracker]:
        ...
        try:
            ...
            if word_tracker.word_stats:
                print(f"📊 Loaded {len(word_tracker.word_stats)} tracked words from: {tracking_file}")
                return word_tracker
            else:
    ...
    def show_abbreviation_opportunities(self, word_tracker: WordTracker, max_candidates: int = 20):
        ...
        if not candidates:
            print("✅ No significant abbreviation opportunities found in tracking data!")
            return []
        
        print(f"\n🎯 ABBREVIATION OPPORTUNITIES FROM WORD TRACKING ({len(candidates)} found)")
        print("=" * 80)
        print(f"{'#':<3} {'Word':<20} {'Freq':<6} {'Savings':<8} {'Suggested':<12} {'Sources'}")
        print("-" * 80)
        ...
        for i, (word, stats) in enumerate(candidates, 1):
            ...
            print(f"{i:<3} {word:<20} {stats.frequency:<6} {stats.total_potential_savings:<8.0f} "
    ...
    def interactive_add_abbreviations(self, candidates: list):
        ...
        for i, (word, stats) in enumerate(candidates, 1):
            ...
            print(f"   💰 Potential savings: {stats.total_potential_savings:.0f} characters")
            ...
            if self.subs_engine:
                try:
                    ...
                    if word.lower() in {v.lower() for v in variants}:
                        print(f"   ✅ Morphological coverage confirmed: '{suggested}' → {word}")
                    else:
                        print(f"   ❌ Morphological coverage issue: '{suggested}' doesn't generate '{word}'")
                        print(f"      Generated variants: {sorted(variants)[:5]}...")
                except Exception as e:
                    print(f"   ❌ Error testing morphological coverage: {e}")
            
            while True:
                choice = input(f"\n   Choose action [1=Accept '{suggested}', 2=Custom, 3=Skip, 4=Quit, ?=Help]: ").strip().lower()
                ...
                if choice == '?':
                    ...
                    while True:
                        ...
                        if not re.match(r'^[a-z][a-z0-9_]*$', custom):
                            print("   ❌ Abbreviation must start with letter and contain only lowercase letters, numbers, and underscores")
                            continue
                        if custom in self.abbreviations:
                            print(f"   ⚠️  Abbreviation '{custom}' already exists with values: {self.abbreviations[custom]}")
                            confirm = input("   Add to existing entry? [y/n]: ").strip().lower()
                            if confirm in ['y', 'yes']:
                                self._add_abbreviation(custom, word)
                                added_count += 1
                                print(f"   ✅ Added to existing: '{custom}' → {self.abbreviations[custom]}")
                                break
                        else:
                            # Test morphological coverage for custom abbreviation
                            if self.subs_engine:
                                try:
                                    variants = self.subs_engine._generate_variants_static(custom)
                                    if word.lower() in {v.lower() for v in variants}:
                                        print(f"   ✅ Custom abbreviation works: '{custom}' → {word}")
                                    else:
                                        print(f"   ⚠️  Custom abbreviation may not cover '{word}'")
                                        print(f"      Generated variants: {sorted(variants)[:5]}...")
                                        confirm = input("   Use anyway? [y/n]: ").strip().lower()
                                        if confirm not in ['y', 'yes']:
                                            continue
                                except Exception as e:
                                    print(f"   ❌ Error testing custom abbreviation: {e}")
                    ...
                    print(f"   🛑 Quitting abbreviation addition (added {added_count} so far)")
                    return added_count
                else:
                    print("   ❌ Invalid choice. Please enter 1, 2, 3, 4, or ? for help.")
    ...
    def check_consolidation_opportunities(self):
        ...
        if not MORPHOLOGICAL_TESTING_AVAILABLE:
            print("❌ SubstitutionEngine not available - cannot analyze consolidation opportunities")
            return []
                    ...
                    if len(covered_values) == len(values_lower):
                        ...
                        for other_key, other_values in self.abbreviations.items():
                            if other_key != key:
                                ...
                                overlap = key_variants_lower.intersection(other_values_lower)
                                if overlap:
    ...
    def interactive_consolidation_implementation(self):
        ...
        if not opportunities:
            print("✅ No consolidation opportunities found - abbreviations are already optimized!")
            return 0
        ...
        for i, opp in enumerate(opportunities, 1):
            ...
            try:
                ...
                print(f"   🧬 **{opp['key']}** morphological variants: {sorted(variants)[:10]}...")
            ...
            while True:
                ...
                if choice in ['y', 'yes', '1']:
                    ...
                    print(f"   🛑 Quitting consolidation (applied {applied_count} so far)")
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            multi_value_keys = sum(1 for values in manager.abbreviations.values() if len(values) > 1)
            ...
            if manager.changes_made:
                confirm = input("⚠️  You have unsaved changes. Exit anyway? [y/n]: ").strip().lower()
                if confirm not in ['y', 'yes']:
                    continue
            print("👋 Goodbye!")
            break
