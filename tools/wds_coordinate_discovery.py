...
class WDSStructureDiscovery:
    ...
    async def analyze_cube_structure(self, product_id: int) -> CubeStructure | None:
        ...
        try:
            ...
            print(
                f"✅ Analyzing product {product_id}: {getattr(metadata, 'cubeTitleEn', 'Unknown')}"
            )
    ...
    def analyze_coordinate_patterns(self) -> dict[str, Any]:
        ...
        dimension_name_patterns: defaultdict[str, list[dict[str, int | str]]] = (
            defaultdict(list)
        )
        ...
        for cube in self.discovered_cubes.values():
            ...
            for dim in cube.dimensions:
                ...
                if any(
                    ...
                    demo_term in dim_name
                    for demo_term in ["age", "sex", "gender", "characteristics"]
                ):
                    ...
                    time_term in dim_name
                    for time_term in ["time", "year", "period", "date"]
                ):
        ...
        for dim in cube.dimensions:
            ...
            for member in dim.members[:20]:  # Limit to first 20 for practicality
                if member["name_en"]:
                    ...
                    member_name = (
                        member["name_en"].upper().replace(" ", "_").replace("-", "_")
                    )
                    member_name = "".join(
                        c for c in member_name if c.isalnum() or c == "_"
                    )
                    members.append(
                        f"    {member_name} = {member['id']}  # {member['name_en']}"
                    )
...
"""
    ...
    async def save_discovery_results(
        ...
        results = {
            ...
            "common_dimensions": {
                k: list(v) for k, v in self.common_dimensions.items()
            },
        }
...
async def main() -> None:
    ...
    print("\n✨ Discovery Complete!")
    print("   • Run this tool to generate coordinate enum files")
    print("   • Results saved for further analysis")
    print("   • Ready to create coordinate-specific enum generators")
