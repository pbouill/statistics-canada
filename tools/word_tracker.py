...
class WordTracker:
    ...
    def track_text_processing(
        ...
        for word in original_words:
            ...
            if (
                len(self.word_stats) >= self.max_tracked_words
                and word not in self.word_stats
            ):
                continue
    ...
    def get_abbreviation_candidates(
        ...
        for word, stats in self.word_stats.items():
            if stats.frequency >= min_frequency and len(word) >= min_length:
    ...
    def generate_abbreviation_report(
        ...
        if not candidates:
            ...
            if format_markdown:
                ...
                for i, (word, stats) in enumerate(candidates[:25], 1):
                    sources_str = ", ".join(sorted(stats.sources))[:30] + (
                        "..." if len(", ".join(stats.sources)) > 30 else ""
                    )
        ...
        if candidates:
            ...
            report_lines.extend(["", "💡 **Suggested Abbreviations for Top 10:**"])
        ...
        if include_contexts and candidates:
            report_lines.extend(["", "🔍 USAGE CONTEXTS (Top 5 words)", "=" * 80])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", type=Path, help="Input JSON file with tracking data")
    parser.add_argument(
        "--output",
        type=Path,
    ...
    parser.add_argument(
        "--min-frequency", type=int, default=3, help="Minimum frequency for candidates"
    )
    parser.add_argument(
        "--min-length", type=int, default=6, help="Minimum word length for candidates"
    )
    parser.add_argument(
        "--max-results",
        type=int,
    ...
    if args.input:
        ...
        report = tracker.generate_abbreviation_report(
            output_file=args.output, include_contexts=args.include_contexts
        )
