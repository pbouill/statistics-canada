...
class EmptyFileChecker:
    ...
    def __init__(self, root_path: Path, include_hidden: bool = False, exclude_dirs: list[str] | None = None, include_intentionally_empty: bool = False):
    ...
    def _should_skip_file(self, file_path: Path) -> bool:
        ...
        if not self.include_intentionally_empty and file_path.name in self.intentionally_empty_files:
            return True
    ...
    def interactive_cleanup(self, categorized_files: dict[str, list[Path]]) -> None:
        ...
        for category, files in categorized_files.items():
            ...
            while True:
                response = input(f"Delete all {category.lower()}? (y/n/s for selective): ").lower().strip()
                if response == 'y':
                    files_to_delete.extend(files)
                    print(f"  ✓ Marked all {len(files)} {category.lower()} for deletion")
                    break
                elif response == 'n':
                    ...
                    for file_path in files:
                        relative_path = file_path.relative_to(self.root_path)
                        while True:
                            file_response = input(f"  Delete {relative_path}? (y/n): ").lower().strip()
                            if file_response == 'y':
...
def main():
    ...
    if not args.include_intentionally_empty:
        print("📝 Note: Excluding intentionally empty files (py.typed, __init__.py, etc.)")
