...
class GeographicEntity:
    ...
    @classmethod
    async def from_member_id(cls, member_id: int, client: WDS | None = None) -> GeographicEntity:
        ...
        for product_id in product_ids:
            try:
                ...
                data = result.vectorDataPoint if hasattr(result, 'vectorDataPoint') else []
    ...
    @classmethod
    async def from_name(cls, name: str, client: WDS | None = None) -> GeographicEntity | None:
    ...
    async def get_population_data(self, client: WDS | None = None, periods: int = 1) -> list[DataPoint]:
        ...
        for product_id in product_ids:
            try:
                result = await client.get_data_from_cube_pid_coord_and_latest_n_periods(
                    product_id=product_id,
                    coordinate=self.coordinate or "",
                    n=periods
                )
                
                # Check if we got valid data
                if (hasattr(result, 'vectorDataPoint') and 
                    result.vectorDataPoint and 
                    len(result.vectorDataPoint) > 0):
    ...
    async def get_data_as_array(self, client: WDS | None = None, periods: int = 10) -> np.ndarray:
    ...
    async def get_data_as_dataframe(self, client: WDS | None = None, periods: int = 10, include_quality_info: bool = True) -> pd.DataFrame:
        ...
        for dp in data:
            ...
            record = {
                ...
                'status_code': dp.statusCode.value if hasattr(dp.statusCode, 'value') else dp.statusCode,
                'status': status_desc,
                'symbol_code': dp.symbolCode.value if hasattr(dp.symbolCode, 'value') else dp.symbolCode,
                'symbol': symbol_desc,
                'scalar_code': dp.scalarFactorCode.value if hasattr(dp.scalarFactorCode, 'value') else dp.scalarFactorCode,
                'scalar': scalar_desc,
                'member_id': self.member_id,
                'location': self.name,
                'coordinate': self.coordinate,
                'release_time': dp.releaseTime,
                'frequency': dp.frequencyCode.name if hasattr(dp.frequencyCode, 'name') else dp.frequencyCode
            }
    ...
    async def get_demographic_dataframe(
        ...
        if demographic_type not in product_map:
            raise ValueError(f"Unknown demographic_type: {demographic_type}. Available: {list(product_map.keys())}")
    ...
    def get_data_quality_info(self, data_point: DataPoint) -> str:
        ...
        if data_point.statusCode:
            ...
            elif data_point.statusCode in [Status.DATA_QUAL_EXCELLENT, Status.DATA_QUAL_VERY_GOOD, Status.DATA_QUAL_GOOD]:
