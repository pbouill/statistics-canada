...
class Coordinate:
    ...
    def _build_parameters(self) -> list[CoordinateParameter]:
        ...
        for i, member_id in enumerate(member_ids):
            ...
            if dimension and member_id != 0:
                ...
                parameters.append(CoordinateParameter(
    ...
    def get_parameter_by_dimension_name(self, dimension_name: str) -> CoordinateParameter | None:
        ...
        **dimension_values: dict[str, str | int]
        ...
        for dim_name, value in dimension_values.items():
            ...
            if member_id is None:
                raise ValueError(f"Member '{value}' not found in dimension '{dim_name}'")
    ...
    def create_demographic_dataframe(
        ...
        for i, (dp, coord) in enumerate(zip(data_points, coordinates)):
            ...
            year_param = coord.get_parameter_by_dimension_name("Census year") or coord.get_parameter_by_position(2)
            characteristic_param = coord.get_parameter_by_position(3)  # Usually age/demographic characteristic
            gender_param = coord.get_parameter_by_dimension_name("Gender") or coord.get_parameter_by_position(4)
            ...
            if hasattr(dp, 'statusCode') and dp.statusCode:
                try:
                    status_enum = Status(dp.statusCode) if isinstance(dp.statusCode, int) else dp.statusCode
            ...
            if hasattr(dp, 'symbolCode') and dp.symbolCode:
                try:
                    symbol_enum = Symbol(dp.symbolCode) if isinstance(dp.symbolCode, int) else dp.symbolCode
            ...
            if hasattr(dp, 'scalarFactorCode') and dp.scalarFactorCode:
                try:
                    scalar_enum = Scalar(dp.scalarFactorCode) if isinstance(dp.scalarFactorCode, int) else dp.scalarFactorCode
            ...
            record = {
                ...
                'geography': geography_param.member_name if geography_param else "Unknown",
                'year_period': year_param.member_name if year_param else str(census_year),
                'characteristic': characteristic_param.member_name if characteristic_param else "Unknown",
                'gender': gender_param.member_name if gender_param else "Total",
                
                # Data value
                'value': float(dp.value) if hasattr(dp, 'value') and dp.value is not None else None,
    ...
    @classmethod
    def _assess_data_quality(cls, data_point: Any, status_enum: Status | None, symbol_enum: Symbol | None) -> str:
                    ...
                    if gender_dim and gender_dim.member:
                        ...
                        for gender_member in gender_dim.member:
                            ...
                            try:
                                ...
                                if (hasattr(data_result, 'vectorDataPoint') and 
                                    data_result.vectorDataPoint and 
                                    len(data_result.vectorDataPoint) > 0):
