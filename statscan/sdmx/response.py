...
class SDMXResponse(Base):
    ...
    def get_annotation(
        ...
        if (
            not structure
            or not hasattr(structure, "annotations")
            or structure.annotations is None
        ):
            return None
    ...
    def get_annotations_by_type(self, annotation_type: str) -> list[Annotation]:
        ...
        if hasattr(structure, "attributes"):
            for attr_list in (
                getattr(structure.attributes, "series", []),
                getattr(structure.attributes, "observation", []),
            ):
    ...
    def get_dimension_summary(self) -> dict[str, dict]:
        ...
        if (
            not structure
            or not getattr(structure, "dimensions", None)
            or not getattr(structure.dimensions, "series", None)
        ):
            return {}
        summary = {}
        for dim in structure.dimensions.series:
            ...
            summary[dim.id] = {
                ...
                "display_name": dim.get_display_name()
                if hasattr(dim, "get_display_name")
                else getattr(dim, "name", dim.id),
                "value_count": len(values),
                "key_position": getattr(dim, "keyPosition", None),
                "roles": getattr(dim, "roles", []) or [],
                "sample_values": [
                    getattr(v, "name", str(getattr(v, "id", ""))) for v in values[:3]
                ],
            }
        return summary
    ...
    def cross_reference_dimension_attribute(
        ...
        if not structure:
            return None, None
        return (
            structure.get_dimension_value_by_attribute(
                dimension_id, attribute_id, value_id
            )
            if hasattr(structure, "get_dimension_value_by_attribute")
            else (None, None)
        )
    ...
    def to_dataframe(self) -> pd.DataFrame:
        ...
        try:
            ...
            dimensions = {}
            if hasattr(structure, "dimensions") and hasattr(
                        ...
                        if i in dimensions:
                            ...
                            try:
                                ...
                                if 0 <= value_index < len(dim_info["values"]):
                                    ...
                                    dimension_values[dim_info["name"]] = getattr(
                                        value_info, "name", f"Value_{value_index}"
                                    )
                            except (ValueError, IndexError):
                                dimension_values[dim_info.get("name", f"dim_{i}")] = (
                                    part
                                )
    ...
    def _standardize_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "Gender" not in df.columns:
            for col in df.columns:
                ...
                if (
                    sample_vals.isin(["total - gender", "total"]).any()
                    or sample_vals.str.contains(
                        "|".join(gender_markers), na=False
                    ).any()
                ):
        ...
        if "Characteristic" not in df.columns:
            ...
            for col in df.columns:
                ...
                score = sum(
                    col_vals.str.contains(term, na=False).sum()
                    for term in characteristic_terms
                )
                if score:
                    candidate_scores.append((col, score))
            if candidate_scores:
                best_col = sorted(candidate_scores, key=lambda x: x[1], reverse=True)[
                    0
                ][0]
        ...
        if {"Characteristic", "Gender", "Value"}.issubset(df.columns):
            pop_mask = (
                df["Characteristic"]
                .astype(str)
                .str.contains("Population, 2021", case=False, na=False)
            )
            if pop_mask.any():
    ...
    def get_population_data(self) -> pd.DataFrame:
        ...
        if "Characteristic" in df.columns:
            mask = (
                df["Characteristic"].astype(str).str.lower().str.contains("population")
            )
            if mask.any():
                return df[mask]
        ...
        for col in df.columns:
            if col not in ["series_key", "time_period", "Value"]:
                col_mask = (
                    ...
                    .astype(str)
                    .str.lower()
                    .str.contains("|".join(population_terms), na=False)
                )
        ...
        for col in df.columns:
            if col not in ["series_key", "time_period", "Value"]:
                col_mask = (
                    ...
                    .astype(str)
                    .str.lower()
                    .str.contains("|".join(age_terms), na=False)
                )
                age_mask |= col_mask
        return df[age_mask]
    ...
    def get_household_statistics(self) -> pd.DataFrame:
        ...
        if "Characteristic" in df.columns:
            mask = (
                ...
                .astype(str)
                .str.lower()
                .str.contains("household|dwelling|private|housing")
            )
            if mask.any():
                return df[mask]
        ...
        for col in df.columns:
            if col not in ["series_key", "time_period", "Value"]:
                col_mask = (
                    ...
                    .astype(str)
                    .str.lower()
                    .str.contains("|".join(household_terms), na=False)
                )
    ...
    def get_attribute_summary(self) -> dict[str, dict]:
        ...
        for attr_list, atype in (
            (getattr(structure.attributes, "series", []) or [], "series"),
            (getattr(structure.attributes, "observation", []) or [], "observation"),
        ):
            for attr in attr_list:
        ...
        if primary_dataset is not None:
            dataset_info = {
                ...
                "total_observations": sum(
                    len(s.observations) for s in primary_dataset.series.values()
                )
                if getattr(primary_dataset, "series", None)
                else 0,
            }
        return {
            "meta": meta,
