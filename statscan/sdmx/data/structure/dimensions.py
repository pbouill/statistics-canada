...
class Dimensions(Base):
    ...
    def get_series_by_key_position(self, key_position: int) -> Optional[Series]:
        ...
        if len(matches) == 0:
            ...
            raise ValueError(f"Multiple series found with key position '{key_position}': {', '.join(s.id for s in matches)}")
        return matches[0]
    ...
    def get_observation_by_name(self, name: str, language: str = 'en') -> Optional[Observation]:
