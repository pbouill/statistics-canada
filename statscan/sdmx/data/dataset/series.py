...
class Series(Base):
    ...
    @staticmethod
    def map_observation(key: str | list[int], observation: list[Optional[float | int]]) -> dict[int, Optional[float | int]]:
    ...
    def map_observations(self, series_key: str) -> dict[int, dict[int, Optional[float | int]]]:
        """Map observations from another series to this series."""
        parsed_key = self.parse_key(series_key)
        return {k: self.map_observation(key=parsed_key, observation=v) for k, v in self.observations.items()}
    ...
    def get_latest_observation(self) -> Optional[tuple[int, list[Optional[float | int]]]]:
    ...
    def get_earliest_observation(self) -> Optional[tuple[int, list[Optional[float | int]]]]:
    ...
    def get_observation_summary(self) -> dict[str, Any]:
        ...
        all_values = [v for obs in self.observations.values() for v in obs if v is not None]
        ...
        summary: dict[str, Any] = {
            ...
            'period_range': (min(self.observations.keys()), max(self.observations.keys())) if self.observations else None,
            'attribute_count': len([attr for attr in self.attributes if attr is not None]),
